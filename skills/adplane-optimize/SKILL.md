...
specific change. When invoked directly, the argument ($ARGUMENTS) narrows
the pass to a platform or a campaign (default: every connected account on
both platforms).
...
cookbook (`references/gaql-cookbook.md` under `adplane-google-ads`) has a
ready query for search terms with spend and no conversions; if it is not
reachable, `google_run_report` on `search_term_view` with a
`metrics.conversions` filter of `eq 0` does the same job.
...
2. **Pause the narrowest thing** that solves the problem:
   `google_update_object` with status PAUSED on a keyword, ad, ad_group,
   or campaign; `meta_update_object` with status PAUSED on an ad, adset,
   or campaign. Never delete when
